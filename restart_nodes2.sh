...
# validator and full node
nohup geth --datadir node1 --networkid $chainid --mine --miner.etherbase $account1 --unlock $account1 --password passwd.txt --allow-insecure-unlock --http --http.api "eth,net,txpool,web3,admin,debug,personal,miner" --http.addr 0.0.0.0 --http.port 9545 -ws --ws.api "eth,net,txpool,web3,admin,debug,personal,miner" --ws.origins "*" --ws.addr 0.0.0.0 --ws.port 9546 --authrpc.port 8552 --port 30304 --http.corsdomain "*" --graphql --syncmode full --config configs/config1.toml --isvs --vscontr "0x3c122a1904F27DD3A2EB9A6C0D1A03C9560527b9" > logs/node1.log 2>&1 &

# validator and full node
nohup geth --datadir node2 --networkid $chainid --mine --miner.etherbase $account2 --unlock $account2 --password passwd.txt --allow-insecure-unlock --http --http.api "eth,net,txpool,web3,admin,debug,personal,miner" --http.addr 0.0.0.0 --http.port 10545 -ws --ws.api "eth,net,txpool,web3,admin,debug,personal,miner" --ws.origins "*" --ws.addr 0.0.0.0 --ws.port 10546 --authrpc.port 8553 --port 30305 --http.corsdomain "*" --graphql --syncmode full --config configs/config2.toml --isvs --vscontr "0x3c122a1904F27DD3A2EB9A6C0D1A03C9560527b9" > logs/node2.log 2>&1 &
